...
module.exports.run = async (client, message, MsgContent, prefix, sqlcon) => {
    ...
    switch (!MsgContent[0] ? MsgContent[0] : MsgContent[0].toString().toLowerCase()) {
        default:
            ...
            break;
        case "add":
            VIPAdd(client, message, this.config.perms[0], MsgContent, sqlcon)
            break;
        case "remove":
        case "del":
            VIPDel(client, message, this.config.perms[0], MsgContent, sqlcon)
            break;
        case "list":
            VIPList(client, message, this.config.perms[0], MsgContent, sqlcon)
            break;
    }
}

async function VIPAdd(client, message, perm, MsgContent, sqlcon) {
...
async function VIPDel(client, message, perm, MsgContent, sqlcon) {
    if (!message.member.hasPermission(perm)) return;
    if (!MsgContent[1]) return message.channel.send("You need to specify a user!");
...
async function VIPList(client, message, perm, MsgContent, sqlcon) {
...
module.exports.config = {
    name: "vip", //Name of the command that will be used to call it
    aliases: ["noping", "pingwarn"], //Aliases of the command that can be used (This must NEVER be left empty)
    info: "Adds a member to the VIP list, meaning that anyone who pings them will be automatically warned", //Short description of the command that will show on all help embeds
